...
```bash
cd /path/to/codey/packages/gateway && source ~/.nvm/nvm.sh && nvm use 22.17.1 && npx tsc -p tsconfig.json
cd /path/to/codey/codey-mac && npx tsc -p tsconfig.electron.json --noEmit
```
