...
**Environment note:** default node is v16 and cannot run vitest/tsc. Prefix every build/test command with `source ~/.nvm/nvm.sh && nvm use v22.17.1 && `. Work in `/Users/jackou/Documents/projects/codey/.worktrees/skill-evolution-trail` (branch `feat/skill-evolution-trail`, stacked on `feat/self-crystallizing-skills`).
