...
- All commands run from `/path/to/codey/codey-mac`.
...
- [ ] **Step 1:** `cd /path/to/codey/codey-mac && source ~/.nvm/nvm.sh && nvm use 22.17.1 && npm run dev`. Kill any pre-existing Codey dev Electron instances first (`pgrep -fl "MacOS/Electron \."`).
